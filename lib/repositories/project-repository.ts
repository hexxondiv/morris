...
export function mapTimelineStages(
  ...
  return stages.map((s) => {
    ...
    return {
      ...
      status: projectStageStatusToApi(s.status),
      planned_start_date: s.plannedStartDate?.toISOString() ?? null,
      planned_end_date: s.plannedEndDate?.toISOString() ?? null,
      actual_start_date: s.actualStartDate?.toISOString() ?? null,
      actual_end_date: s.actualEndDate?.toISOString() ?? null,
